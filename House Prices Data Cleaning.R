...
colnames(hh2021) = c("ID" , "Price", "Year", "PostCode" , "PAON", "SAON", "FL", "House Num", "Flat", "Street Name",
                     "Locality", "Town" , "District", "County" , "Type1", "Type2")
HousePrices = rbind(hh2019,hh2020,hh2021) %>% 
  na.omit() %>% 
  distinct() %>% 
  as_tibble()
View(HousePrices)

write.csv(HousePrices, "Cleaning/cleaned datasets/Combined_House_Pricing_2019-2021.csv")

# Filtering Greater Manchester and Merseyside data
FilteredHousePrices = filter(HousePrices, County == 'LEICESTERSHIRE' | County == 'LANCASHIRE')
pattern = ' .*$'
...
write.csv(FilteredHousePrices, "Cleaning/Cleaned datasets/Cleaned_House_Pricing_2019-2021.csv",row.names = FALSE)
